#!/usr/bin/env python3
"""Create a controlled ICA component-review package for BIDS/EEGLAB data."""

from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(PROJECT_ROOT / "src"))

from hunt_eeg.bids_eeglab import BIDSEeglabInputs
from hunt_eeg.bids_eeglab_ica import publish_ica_review
from hunt_eeg.ica import DEFAULT_ICA_CONFIG


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--dataset-root", type=Path, required=True)
    parser.add_argument("--set", dest="raw_set", type=Path, required=True)
    parser.add_argument("--profile", type=Path, required=True)
    parser.add_argument("--qc", type=Path, required=True)
    parser.add_argument("--review-bundle", type=Path, required=True)
    parser.add_argument("--participant-id", required=True)
    parser.add_argument("--ica-config", type=Path, default=DEFAULT_ICA_CONFIG)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    output = publish_ica_review(
        BIDSEeglabInputs(args.dataset_root, args.raw_set),
        args.profile,
        args.qc,
        args.review_bundle,
        args.participant_id,
        args.output,
        args.ica_config,
    )
    summary = json.loads((output / "ica_review_summary.json").read_text())
    print(json.dumps({"status": summary["status"], "output": str(output)}))


if __name__ == "__main__":
    main()
